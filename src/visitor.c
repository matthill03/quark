...
static ast_T *builtin_function_echo(visitor_T *visitor, ast_T **args,
                                    int args_size) {
...
ast_T *visitor_visit_variable_definition(visitor_T *visitor, ast_T *node) {
  ...
  if (visitor->variable_definitions == (void *)0) {
    ...
    visitor->variable_definitions[visitor->variable_definitions_size - 1] =
        node;
  }
...
ast_T *visitor_visit_variable(visitor_T *visitor, ast_T *node) {
  for (int i = 0; i < visitor->variable_definitions_size; i++) {
    ...
    if (strcmp(vardef->variable_definition_variable_name,
...
ast_T *visitor_visit_function_call(visitor_T *visitor, ast_T *node) {
  if (strcmp(node->function_call_name, "echo") == 0) {
    return builtin_function_echo(visitor, node->function_call_arguments,
                                 node->function_call_arguments_size);
  }

  printf("Undefined function '%s'", node->function_call_name);
}

ast_T *visitor_visit_string(visitor_T *visitor, ast_T *node) { return node; }
